...
def download_pdfs(query,count):
    ...
    get_pdfs(article_pmids[:count]) # make this a user input
    remove_unnecessary_directories()
    take_out_trash()

def initialize():
    #rm existing fetched_pdfs dir
    try:
...
def create_database():
    ...
    for file in os.listdir(download_dir):
        ...
        # you probably do not want the stuff in the references so I remove them from tokens to save time later in analysis.
